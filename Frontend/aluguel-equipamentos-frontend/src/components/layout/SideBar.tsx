...
const Sidebar = ({ sidebarOpen }: any) => {
    const mainMenuItems: MenuItemProps[] = [
        ...
        },
        {
            ...
            label: "Categorias equipamento",
            link: "/categorias-equipamento"
        },
        {
            icon: (
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
    ...
    return (
        <aside
            ...
            className={`fixed border top-0 left-0 z-40 w-64 h-screen transition-transform ${sidebarOpen ? "translate-x-0" : "-translate-x-full"
                } sm:translate-x-0`}
            aria-label="Sidebar"
        >
                ...
                <ul className="space-y-2 font-medium">
                    {mainMenuItems.map((item, index) => (
                        <li key={`main-${index}`}>
                            <a
                                href={item.link}
                                className="flex items-center p-2 text-dark rounded-lg hover:text-indigo-600 hover:bg-gray-50 group"
                            >
                                {item.icon}
                                <span className="flex-1 ms-3 text-sm whitespace-nowrap">{item.label}</span>
                                {item.badge && (
                                    <span className={`inline-flex items-center justify-center px-2 ms-3 text-sm font-medium text-${item.badgeColor || 'gray'}-800 bg-${item.badgeColor || 'gray'}-100 rounded-full dark:bg-gray-700 dark:text-gray-300`}>
                                        {item.badge}
                                    </span>
                                )}
                            </a>
